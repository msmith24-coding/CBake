...
std::vector<std::string> Parser::makeSimpleCompileFunc()
{
    ...
    if(this->currentToken.getType() == TT_LPAREN) {
        ...
        if(compiler.empty()) {
            this->throwError("simple_compile -> (compiler, objs, out, compiler_flags).");
        }
        ...
        this->advance();
        std::string out = this->makeStringArg(TT_COMMA);
        ...
        this->advance();
        std::string cFlags = this->makeStringArg(TT_RPAREN);
